...
void SpikeGenerator(triangle VS_DATA vertices[3], inout TriangleStream<GS_DATA> triStream)
{
    ...
    basePoint = float3(vertices[0].Position + vertices[1].Position + vertices[2].Position) / 3;
	//Step 2. Calculate The normal of the basePoint
    float3 normalBasePoint = (vertices[0].Normal + vertices[1].Normal + vertices[2].Normal) / 3;//float
	//Step 3. Calculate The Spike's Top Position
    top = basePoint + (8 * normalBasePoint);
	//Step 4. Calculate The Left And Right Positions
    float3 spikeDirection = ((vertices[2].Position - vertices[0].Position) * 0.1f); //float
