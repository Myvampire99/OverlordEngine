...
void SpriteRenderer::UpdateBuffer(const GameContext& gameContext)
{
	...
	///memcpy(mappedResource.pData, m_pVertexBuffer, sizeof(SpriteVertex)* m_Sprites.size());
	///memcpy( m_pVertexBuffer, &mappedResource.pData, sizeof(SpriteVertex)* m_Sprites.size());
	int bufferPosition = 0;
	for each (SpriteVertex pSprite in m_Sprites)
	{
		((SpriteVertex*)mappedResource.pData)[bufferPosition] = pSprite;
		++bufferPosition;
	}//======== ???
