...
void ControllerComponent::Initialize(const GameContext&)
{
	...
	physx::PxControllerManager* conMan = GetGameObject()->GetScene()->GetPhysxProxy()->GetControllerManager();
	//TODO: 2. Create a PxCapsuleControllerDesc (Struct)
	physx::PxCapsuleControllerDesc CapDesc;
	//  > Call the "setToDefault()" method of the PxCapsuleControllerDesc
	CapDesc.setToDefault();
	//	> Fill in all the required fields
	CapDesc.setToDefault();
	CapDesc.radius = m_Radius;
	CapDesc.height = m_Height;
	CapDesc.climbingMode = m_ClimbingMode;
	CapDesc.upDirection = physx::PxVec3(0, 1, 0);
	CapDesc.contactOffset = 0.1f;
	CapDesc.material = m_pMaterial;
	CapDesc.position = (physx::PxExtendedVec3(m_pGameObject->GetTransform()->GetPosition().x, m_pGameObject->GetTransform()->GetPosition().y , m_pGameObject->GetTransform()->GetPosition().z));
	CapDesc.userData = this;
	...
	if (CapDesc.isValid())
		m_Controller = conMan->createController(CapDesc);
